...
def diff(inst1, inst2):
    ...
    considered.  Each difference is a ``(name, value_in_inst1,
    value_in_inst2)`` tuple, in field definition order.  Fields excluded from
    comparison never appear in the result.
    ...
    Returns:
        list[tuple[str, typing.Any, typing.Any]]: A list of differences;
        empty when the instances are equal by their comparison fields.
    ...
    for a in fields(inst1.__class__):
        ...
        value1 = getattr(inst1, a.name)
        value2 = getattr(inst2, a.name)
        if value1 != value2:
            differences.append((a.name, value1, value2))
